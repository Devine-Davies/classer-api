...
@section('content')
    <section class="admin-card max-w-3xl overflow-hidden h-full flex flex-col">
        @include('admin.shipping.partials.form', [
            'item' => $item,
            'isEdit' => true,
            'action' => route('admin.shipping.update', ['shippingRow' => $item['_row']]),
            'method' => 'PUT',
        ])
    </section>
    ...
    <section class="admin-card max-w-3xl overflow-hidden">
        <div class="p-5 flex items-center justify-between gap-4">
            <div>
                <h3 class="text-base font-bold text-slate-900">Delete shipping row</h3>
                <p class="mt-1 text-sm text-slate-500">This removes the selected row from shipping.json.</p>
            </div>

            <form method="POST" action="{{ route('admin.shipping.destroy', ['shippingRow' => $item['_row']]) }}" onsubmit="return confirm(@js('Delete this shipping row? This cannot be undone.'))">
                @csrf
                @method('DELETE')
